...
PROJECT_ID   = "sadegh-sb"                   # gcloud project ID
ZONE         = "us-east1-c"               # Must support C3D (c3d-standard-*)
...
SSH_KEY_PATH     = r"~/.ssh/gcp_sbcli"        # ed25519 private key (no passphrase)
SSH_PUB_KEY_PATH = r"~/.ssh/gcp_sbcli.pub"   # Public key — injected into instance metadata
SSH_USER         = "sbadmin"                  # Linux user GCP creates from key metadata

BRANCH   = "chris-benchmark"
MAX_LVOL = "50"
IFACE    = "eth0"                             # Primary NIC name on GCP VMs (c3d instances use eth0)
DATA_CHUNKS_PER_STRIPE = 1
PARITY_CHUNKS_PER_STRIPE = 1
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Deploy simplyblock perf test cluster on GCP."
    )
    parser.add_argument(
        "--data-chunks-per-stripe",
        type=int,
        default=DATA_CHUNKS_PER_STRIPE,
        dest="data_chunks_per_stripe",
        help="Data chunks per stripe for cluster create (default: 1).",
    )
    parser.add_argument(
        "--parity-chunks-per-stripe",
        type=int,
        default=PARITY_CHUNKS_PER_STRIPE,
        dest="parity_chunks_per_stripe",
        help="Parity chunks per stripe for cluster create (default: 1).",
    )
    return parser.parse_args()


def main(data_chunks_per_stripe: int, parity_chunks_per_stripe: int):
    print("=" * 60)
    ...
    print(f"  Stripe  : data={data_chunks_per_stripe}, parity={parity_chunks_per_stripe}")
    print("=" * 60)
    ...
    install_cmds = [
        "sudo dnf install git python3-pip nvme-cli pciutils nano tmux -y",
    ...
    ssh_exec(mgmt_pub_ip, [
        ...
        f" --data-chunks-per-stripe {data_chunks_per_stripe}"
        f" --parity-chunks-per-stripe {parity_chunks_per_stripe}"
    ], check=True)
    ...
    final_metadata = {
        ...
        "zone": ZONE,
        "subnet": SUBNET,
        "data_chunks_per_stripe": data_chunks_per_stripe,
        "parity_chunks_per_stripe": parity_chunks_per_stripe,
...
if __name__ == "__main__":
    args = parse_args()
    main(args.data_chunks_per_stripe, args.parity_chunks_per_stripe)
